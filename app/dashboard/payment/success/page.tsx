  ...
  useEffect(() => {
    ...
    const verifyPayment = async () => {
      try {
        ...
        // Preload refreshed subscription state so dashboard shows credits immediately
        try {
          await fetch("/api/search/status", { cache: "no-store" });
        } catch {
          // non-blocking
        }
        // Dispatch event to refresh user data in navbar
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-gray-900">
                    {paymentDetails.packageName || userDetails?.packageName || "Pay As You Go"}
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-gray-900">
                    ...
                    {Number(
                      paymentDetails.amount ??
                        userDetails?.lastPaymentAmount ??
                        0
                    ).toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Credits / Searches:</span>
                  <span className="font-medium text-gray-900">
                    {paymentDetails.creditsPurchased ??
                      paymentDetails.searchesIncluded ??
                      userDetails?.subscription?.searchLimit ??
                      0}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                {userDetails?.packageEndsAt && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">Expires:</span>
                    <span className="font-medium text-gray-900">
                      {new Date(userDetails.packageEndsAt).toLocaleDateString()}
                    </span>
                  </div>
                )}
              </div>
