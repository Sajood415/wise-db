...
const DashboardHeader = ({
  ...
  // Determine which expiration date to use (include enterprise packages and PAYG)
    ...
    <header className="bg-white border-b border-gray-200 px-6 py-4">
              ...
              {user?.subscription?.type === "pay_as_you_go" ? (
                <span className="inline-flex items-center gap-2 text-xs px-3 py-1 rounded-full border bg-blue-50 text-blue-700 border-blue-200">
                  ...
                  <span>
                    ...
                    {packageEndsAt
                      ? ` · Expires ${formatDate(packageEndsAt)}`
                      : ""}
                  </span>
                </span>
              ) : expirationDate &&
                ...
                <span className="inline-flex items-center gap-2 text-xs px-3 py-1 rounded-full border bg-rose-50 text-rose-700 border-rose-200">
                  ...
                  <span>
                    {trialActive ? "Free trial expired" : "Package expired"}
                  </span>
                </span>
              ) : (
                <span className="inline-flex items-center gap-2 text-xs px-3 py-1 rounded-full border bg-blue-50 text-blue-700 border-blue-200">
      ...
      {shouldShowPackageCapsule && (
        <div className="md:hidden px-6 py-2 border-b border-gray-200">
          {/* Don't show expiration for pay-as-you-go */}
          {user?.subscription?.type === "pay_as_you_go" ? (
            ...
            >
              ...
              <span>
                {trialActive ? "Trial" : user?.packageName || "Package"}:{" "}
            ...
            <div className="inline-flex items-center gap-2 text-xs px-3 py-1 rounded-full border bg-blue-50 text-blue-700 border-blue-200">
              ...
              <span>
                {user?.subscription?.type === "pay_as_you_go"
                  ...
                  : `${user?.packageName || "Package"}: Active`}
              </span>
            </div>
          )}
        </div>
...
export default function DashboardLayout({
  ...
  // Listen for custom event to refresh user data (e.g., from payment success)
  useEffect(() => {
  ...
  // Refresh user data when returning to dashboard from payment pages
  useEffect(() => {
    if (pathname === "/dashboard") {
      // Refresh when landing on main dashboard page
      // This ensures navbar updates after returning from payment success
      const timer = setTimeout(() => {
        fetchUserProfile();
      }, 500); // Small delay to ensure payment processing is complete
