  ...
  try {
    ...
    if (!packageId || !packageName || !packageType) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: packageId, packageName, or packageType",
        },
        { status: 400 }
      );
    }
    ...
    const trialExtensionDays =
      packageType === "pay_as_you_go" ? 0 : packageType === "yearly" ? 365 : 30;
    ...
    if (packageType === "pay_as_you_go" && packageName !== "Pay As You Go") {
      return NextResponse.json(
        {
          error:
            'Package name must be "Pay As You Go" for pay_as_you_go package type',
        },
        { status: 400 }
      );
    }
    ...
    const paymentData = {
      ...
      metadata: {
        ...
        features:
          packageType === "pay_as_you_go"
            ? ["Pay as you go - $2 per search", "Access to real fraud data"]
            : [
